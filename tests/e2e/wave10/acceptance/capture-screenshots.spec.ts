...
test.describe('Wave 10 acceptance screenshots', () => {
  ...
  test('8 demo screenshots (real claude CLI)', async ({ browser }) => {
    ...
    try {
      ...
      await harness.sendMessage('只输出数字 42, 不要其他任何内容, 不要标点, 不要引号');
      ...
      await page.locator('.aui-assistant-message-content').first().waitFor({ timeout: 60_000 });
      ...
      await rail
        .locator('button[aria-label^="Claude Code"]')
        .click()
        .catch(() => {});
