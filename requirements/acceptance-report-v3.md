# QRClaw V3 Acceptance Test Report
...
| **Skipped** | 6 |
| **Failed** | 0 |
| **Pass rate** | 96.1% (146/152) |
| **Effective rate** | 100% (146/146 non-skipped) |
| **Duration** | ~1.7 min |

### Comparison: V1 → V2 → V3

| | V1 | V2 | V3 | V2→V3 Delta |
|---|---|---|---|---|
| Passed | 86 | 142 | 146 | **+4** |
| Skipped | 102 | 10 | 6 | **-4** |
| Failed | 0 | 0 | 0 | — |

### V3 Newly Passing Tests (+4)

| Test | Module | What Changed |
|------|--------|--------------|
| K-01 | Conversation | Gateway `persistMessage()` now writes to Supabase `messages` table |
| K-02 | Conversation | Message record verified: encrypted content, role, encryption_meta |
| K-03 | Conversation | Multiple messages ordered by `sent_at` timestamp |
| F-09 | Visitor Chat | RegisterBanner component visible with "Sign up" link |
...
### Module F: Visitor Chat UI (14 tests — 12 passed, 2 skipped)
...
| F-09 | ✅ | **RegisterBanner prompts visitor to sign up** _(NEW in V3)_ |
...
### Module K: Conversation (7 tests — 7 passed, 0 skipped)

| Test | Status | Description |
|------|--------|-------------|
| K-01 | ✅ | **Message persistence via WS → DB** _(NEW in V3)_ |
| K-02 | ✅ | **Message record fields (encrypted, role, meta)** _(NEW in V3)_ |
| K-03 | ✅ | **Message ordering by sent_at** _(NEW in V3)_ |
...
All skipped tests have documented reasons — they require unimplemented features or are not testable locally:
...
1. **Gateway `persistMessage()`** — Full encryption-to-DB pipeline:
   - AES-256-GCM envelope encryption via existing crypto module
   - Upserts `encryption_keys` record per conversation (with caching)
   - Inserts encrypted message to `messages` table via Supabase REST API
   - Conversation ID resolution: maps WS `qrCodeId` → DB `conversations.id`
...
2. **RegisterBanner (F-09)** — Already implemented in V2, test activated:
   - Component at `web/src/components/chat/RegisterBanner.tsx`
   - Shows "Sign up to save chats" with link to `/signup` and dismiss button

3. **DB Schema Migration** — File written at `supabase/migrations/20260315_schema_additions.sql`:
   - `agents.description TEXT`
   - `qrcodes.name VARCHAR(255)`
   - `conversations.status VARCHAR(20) DEFAULT 'active'`
   - `CREATE TABLE visitor_sessions` with RLS
   - Note: Requires Supabase Dashboard or access token to apply DDL

### New Files Created
| File | Purpose |
|------|---------|
| `gateway/src/db/supabase.ts` | Supabase admin client (service_role) |
| `gateway/src/db/persist.ts` | Message persistence: upsert key + insert message + conversation resolution |
| `supabase/migrations/20260315_schema_additions.sql` | Schema additions migration |

### Files Modified
| File | Change |
|------|--------|
| `gateway/src/ws/router.ts` | `persistMessage()` now calls `persistEncryptedMessage()` with real DB writes |
...
| f-visitor-chat.spec.ts | 14 | 12 | 2 |
